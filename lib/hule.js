...
function get_post_hupai(shoupai, rongpai, baopai, fubaopai, kita = 0,
                        sanma = false) {
    ...
    let n_baopai = 0;
    for (let p of baopai) {
        p = Majiang.Shan.zhenbaopai(p, sanma);
    ...
    if (kita) post_hupai.push({ name: '北ドラ', fanshu: kita });
...
function get_defen(fu, hupai, rongpai, param) {
    ...
    let sanma = param.rule['三人打ち'] || param.rule['人数'] === 3;
    let n_players = sanma ? 3 : 4;
    ...
    if (hupai[0].fanshu[0] == '*') {
        ...
        if (h) {
            baojia2 = (menfeng + { '+': 1, '=': 2, '-': 3}[h.baojia]) % n_players;
    ...
    let fenpei  = sanma ? [ 0, 0, 0 ] : [ 0, 0, 0, 0 ];
    ...
    if (rongpai || base == 0) {
        baojia = (base == 0)
                    ? baojia2
                    : (menfeng + { '+': 1, '=': 2, '-': 3}[rongpai[2]]) % n_players;
        ...
        if (sanma) {
            // Honba is paid in whole 100-point units per payer. With two payers
            // the winner receives 100 x 2 per honba, not the yonma total of 300.
            let honbaPerPayer = chang * 100;
            let honbaTotal    = honbaPerPayer * 2;
            if (menfeng == 0) {
                defen = zhuangjia * 2;
                for (let l = 0; l < 3; l++) {
                    if (l == menfeng)
                            fenpei[l] += defen     + honbaTotal + lizhi * 1000;
                    else    fenpei[l] -= zhuangjia + honbaPerPayer;
                }
            }
            else {
                defen = zhuangjia + sanjia;
                for (let l = 0; l < 3; l++) {
                    if (l == menfeng)
                            fenpei[l] += defen     + honbaTotal + lizhi * 1000;
                    else if (l == 0)
                            fenpei[l] -= zhuangjia + honbaPerPayer;
                    else    fenpei[l] -= sanjia    + honbaPerPayer;
                }
            }
        }
        else {
...
function hule(shoupai, rongpai, param) {
    ...
    let post_hupai = get_post_hupai(shoupai, rongpai,
                                    param.baopai, param.fubaopai, param.kita,
                                    sanma);
